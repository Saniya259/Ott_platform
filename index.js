...
const con = require('./DB_Conn.js'); // Your database connection module.module is inbuilt  package
...
app.post('/RegistrationValidation', (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        const sql = "INSERT INTO user (name, email, password, address, gender) VALUES (?, ?, ?, ?, ?)";
        const values = [name, email, hashedPassword, address, gender];
...
app.post('/LoginValidation', (req, res) => {
    ...
    con.query(sql, [uname], function (err, result) {
        ...
        bcrypt.compare(pass, storedHash, function (err, isMatch) {
            ...
            if (isMatch) {
                res.sendFile(path.join(publicpath, 'ott.html')); // Successful login
            } else {
